...
function StatusPill({ label }: { label: string }) {
  const cls = STAGE_COLORS[label] ?? "bg-slate-100 text-slate-600 border-slate-300";
  return <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold whitespace-nowrap border ${cls}`}>{label}</span>;
}
