...
async function runTest() {
  ...
  for (const q of allQuotes) {
    console.log(`- Quote v${q.version} (${q.quoteNumber}): Status=${q.status}, isFinalAgreed=${(q as any).isFinalAgreed}, Amount=${q.totalAmount}`);
  }
  ...
  if ((allQuotes[2] as any).isFinalAgreed !== true) throw new Error("Expected Quote v3 to have isFinalAgreed=true");
  if ((allQuotes[0] as any).isFinalAgreed === true || (allQuotes[1] as any).isFinalAgreed === true) {
