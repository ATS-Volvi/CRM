...
async function runTests() {
  ...
  console.log("  Deal A Status:", (dealA as any).status);
  ...
  console.log("  Deal B Status:", (dealB as any).status);
  ...
  if (dealB.stageId === wonStage.id || (dealB as any).status === "WON") {
  ...
  console.log("  Updated Deal B Status:", (dealB as any).status);
  ...
  console.log("  Updated Deal C Status:", (dealC as any).status);
  console.log("  Updated Deal C Loss Reason:", (dealC as any).lossReason);

  if (poCUpdated?.status !== "Rejected") {
    throw new Error(`Expected PO status 'Rejected' after reject resolution, got '${poCUpdated?.status}'`);
  }
  if ((dealC as any).status !== "LOST") {
    throw new Error("Expected deal to be moved to LOST after REJECT_LOST resolution.");
  }
  if (!(dealC as any).lossReason || !(dealC as any).lossReason.includes("Commercial Variance")) {
